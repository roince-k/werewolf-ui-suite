...
import { ArrowLeft, X, Plus, Upload, Sparkles, Brain, MessageSquare, Swords, User, Pencil, Trash2 } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="h-14 border-b border-border flex items-center px-4 gap-4">
        ...
        <h1 className="display-title text-xl text-foreground">🤖 智能体模板</h1>
        <div className="flex-1" />
        <button
          onClick={() => setShowCreate(true)}
          className="btn-ritual flex items-center gap-2 text-sm py-2"
        >
      ...
      <div className="max-w-6xl mx-auto p-6">
        <p className="text-muted-foreground mb-8">
          选择一个 AI 智能体模板，在游戏中 AI 玩家将以该模板的风格进行发言和推理。你也可以创建自定义智能体。
        </p>
...
const CreateAgentModal = ({ onClose, onCreate }: { onClose: () => void; onCreate: (agent: Omit<AgentTemplate, 'id'>) => void }) => {
  ...
  const buildPrompt = () => {
    return `你是一个狼人杀游戏中的AI玩家，名字叫"${name}"。\n\n【游戏风格】${gameStyle}\n【角色性格】${personality}\n【推理方式】${reasoning}\n【语言风格】${languageStyle}\n\n请始终按照以上风格进行发言、推理和投票。`;
  };
            ...
            return (
              <div key={key}>
                ...
                <textarea
                  ...
                  rows={2}
                  className="input-ritual text-sm min-h-0 resize-none"
                />
              </div>
            );
          })}

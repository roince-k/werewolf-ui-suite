    ...
    <div ref={containerRef} className="bg-background text-foreground">
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        >
          ...
          >
            MIND<span className="text-werewolf text-glow-werewolf">ARENA</span>
          </motion.h1>
      ...
      <section id="values" className="relative py-24 sm:py-32">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <SectionHeader
            label="CORE VALUES"
            title="平台核心价值"
            subtitle="无需主持人，无需组织，AI 驱动的全新博弈体验"
          />
            ...
            {[
              {
                ...
                title: '自动化的游戏流程',
                desc: '无需主持人，无需房主。由系统自动推进阶段、结算与提示。',
                accent: 'werewolf' as const,
              },
              {
                icon: <Bot className="w-7 h-7" />,
                title: 'AI Agent 参与博弈',
                desc: '支持 AI 角色像真实玩家一样加入游戏：独立身份、独立推理、独立行为决策。',
                accent: 'moonlight' as const,
              },
              {
                icon: <Puzzle className="w-7 h-7" />,
                title: '通用博弈平台架构',
                desc: '狼人杀、剧本杀、社交小游戏。支持更多类型的社交推理与阵营博弈游戏。',
      ...
      <section className="relative py-24 sm:py-32 overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6">
          <SectionHeader
            label="FEATURED GAME"
            title="当前游戏：狼人杀"
            subtitle="从经典玩法开始，体验 AI 驱动的社交推理"
          />

          <div className="mt-16 grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {[
              { icon: <Swords className="w-6 h-6" />, title: '标准规则自动化', desc: '完整的狼人杀规则引擎，自动推进每个阶段' },
              { icon: <Users className="w-6 h-6" />, title: '多人实时对局', desc: '真人与 AI 混合参与，随时开局无需等待' },
              { icon: <Shield className="w-6 h-6" />, title: '多种角色配置', desc: '预言家、女巫、守卫等经典角色一应俱全' },
              { icon: <Eye className="w-6 h-6" />, title: '智能 AI 对手', desc: 'AI 具备独立推理能力，带来真实博弈体验' },
            ].map((feat, i) => (
              <motion.div
                key={feat.title}
                variants={fadeUp}
            ...
            {[
              {
                ...
                title: '多类型社交推理游戏',
                desc: '抵抗组织、阿瓦隆、捉鬼等经典社交推理游戏即将登场。',
                status: 'PLANNED',
              },
              {
                icon: <Sparkles className="w-6 h-6" />,
                title: 'AI 生成的原创博弈玩法',
                desc: '基于平台规则系统生成新游戏模式。AI 的互相博弈，人类玩家也可以参与或观察。',
                status: 'EXPLORING',
              },
              {
                icon: <Theater className="w-6 h-6" />,
                title: 'Game Master',
                desc: '未来加入 AI 主持人，用于非固定流程的游戏，打造更丰富的叙事体验。',
                status: 'FUTURE',
              },
            ].map((item, i) => (
              <motion.div
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-2 flex-wrap">
                    ...
                    <span className="text-[10px] tracking-wider uppercase px-2 py-0.5 rounded-full border border-moonlight/20 text-moonlight-dim">
                      {item.status}
                    </span>
                  </div>
      ...
      <section className="py-24 sm:py-32">
        ...
        >
          <h2 className="display-title text-4xl sm:text-5xl text-foreground text-glow-moonlight mb-4">
            准备好了吗？
          </h2>
          <p className="text-muted-foreground mb-8">加入 MindArena，开启你的 AI 博弈之旅。</p>
          <button onClick={() => navigate('/auth')} className="btn-ritual text-lg animate-pulse-glow">
            立即开始
          </button>
        </motion.div>
      ...
      <footer className="border-t border-border/50 py-12">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 flex flex-col sm:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-3">
            <span className="display-title text-xl text-foreground">MIND<span className="text-werewolf">ARENA</span></span>
          ...
          <p className="text-muted-foreground text-xs">© 2026 MindArena. All rights reserved.</p>
        </div>
      </footer>
    </div>
  );
...
function ValueCard({
  ...
  icon: React.ReactNode; title: string; desc: string; accent: 'werewolf' | 'moonlight' | 'gold'; index: number;
}) {
  const accentMap = {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true, margin: '-60px' }}
      custom={index}
      className={`surface-elevated rounded-xl p-6 transition-colors duration-300 ${a.border}`}
    >
      <div className={`w-12 h-12 rounded-xl ${a.bg} ${a.text} flex items-center justify-center mb-5`}>
        {icon}
      </div>
